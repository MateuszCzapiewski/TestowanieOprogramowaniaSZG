...
class CarModelTest(TestCase):
        ...
        def setUp(self):
            self.car = Car.objects.create(
                marka='Audi',
                model='A4',
                rok=date.today() - timedelta(days=365 * 5),  # 5 lat temu
                automat=True,
                cena=50000,
                dostępność=True
            )
        ...
        def test_str_representation(self):
            self.assertEqual(str(self.car), 'Audi')

        def test_is_old_car(self):
            self.assertTrue(self.car.is_old_car())

        def test_car_representation(self):
            expected_representation = 'Audi by A4 by {} by 50000'.format(self.car.rok)
            self.assertEqual(self.car.car_representation(), expected_representation)

        def test_get_absolute_url(self):
            expected_url = f'/ai_app/car/{self.car.pk}/'
            self.assertEqual(self.car.get_absolute_url(), expected_url)

        def test_reserve(self):
            print(f"Before reserve: {self.car.dostępność}")
            self.car.reserve()
            print(f"After reserve: {self.car.dostępność}")
            self.assertFalse(self.car.dostępność)
